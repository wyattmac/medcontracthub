          ...
          for (const opp of response.opportunitiesData) {
            try {
              const transformedOpp = {
                ...
                sub_agency: opp.subtierAgency || '',
                office: opp.office || '',
                posted_date: opp.postedDate,
                response_deadline: opp.responseDeadLine || opp.responseDeadline || opp.archiveDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
                archive_date: opp.archiveDate,
                set_aside_type: opp.typeOfSetAside,
                contract_type: opp.type,
                naics_code: opp.naicsCode || naicsCode,
                naics_description: Array.isArray(opp.naicsCodes) && opp.naicsCodes[0]?.value || '',
